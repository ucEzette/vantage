...
**Dual-Chain Design:** Hedera handles the internal token economy (Pulse, governance), while x402 + Circle Nanopayments on Arc handles inter-Vantage service commerce (USDC nanopayments). All revenue stays in the Agent Treasury. No overlap — different chains, different tokens, different purposes.
...
│ Governance         │  │ Inter-Vantage Commerce (USDC)   │
│                    │  │ Agent Treasury (USDC)           │
└────────────────────┘  └────────────────────────────────┘
```
...
|---|---|---|
| **Vantage Web** | Vercel | UI, API, Vantage registration, Pulse issuance, Commerce Storefront |
| **Database** | Supabase | Vantage metadata, activity logs, revenue records, commerce queue |
...
| **Arc (EVM L1)** | Decentralized | Circle Nanopayments + x402, inter-Vantage commerce (USDC), Agent Treasury management |
...
| **Scope** | Vantage ↔ Patron (governance participants) | Vantage ↔ Vantage (trading partners) |
...
|---|---|---|---|
| Vantage Genesis Engine | Web (on-chain) | Vantage registration + Pulse token issuance via VantageRegistry contract (Creator signs, HTS precompile mints token, 3% launchpad fee to protocol wallet) | Next.js, Solidity, HTS Precompile |
...
| Commerce Storefront | Web | Per-Vantage x402 service endpoint + job queue | Next.js API Routes, Supabase |
| Prime Agent Runtime | Local | Autonomous GTM execution via tool-calling agent loop | OpenAI SDK, Stagehand, Hedera Agent Kit |
| Inter-Vantage Commerce | Local + Web | x402 signing on Arc (Local via Circle Wallets) + Storefront/job queue (Web) | x402, Circle Nanopayments, Circle Developer-Controlled Wallets |
